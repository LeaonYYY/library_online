            ...
            <div id="bookDetail_head" style="margin-left:60px">
                ...
                <div style="display:flex;flex-direction:column;justify-content:space-around;margin-left:60px">
                    ...
                    <div style="height:90px;width:300px;display:flex;flex-direction:row;justify-content:space-between">
                        ...
                        <div id="btn_lend"><el-button @click="handleBorrow(pickBook.ID)">借阅</el-button></div>
                    </div>
...
export default {
  ...
  methods: {
    serch_Book () {
      this.$axios({
        url: '/api/v1/book/search?bookname='+this.serchBook+'&pagesize=2&pagenum=1',
        method: 'post'
      }) .then((res) =>{
    ...
    handleCurrentChange (val) {
      this.itemsMode =[]
       this.$axios({
          url: '/api/v1/books/up?pagesize=7&pagenum='+val,
          method: 'get'
        }) .then((res)=>{
          console.log(res);
          for(let i=0;i<res.data.data.length;i++){
    ...
    handleBorrow(id){
      this.$axios({
        url: '/api/v1/user/book/borrow/'+id,
        method: 'post',
        headers: {
          'Authorization': "Bearer "+localStorage.getItem('token1')
        }
      }) .then((res) =>{
        if(res.data.status === '<nil>'){
          this.$message({
            message: '借阅成功',
            type: 'success'
          })
  ...
  mounted () {
    this.$axios({
      url: '/api/v1/books/up?pagesize=7&pagenum=1',
      method: 'get'
    }) .then((res)=>{
      for(let i=0;i<res.data.data.length;i++){
        this.$set(this.itemsMode,i,res.data.data[i])
